...
index_bray = vegdist(t(index[, 1:ncol(index)]), method = "bray") 
...
plot_pro = rbind(INA_sp, INB_sp) #plot_pro = rbind(INA_sp, A,INB_sp, B) 
...
ggplot(plot_pro, aes(MDS1, MDS2, color = Time, shape = method)) + 
    geom_point(size = 3) +
    geom_line(aes(group = Sample, color = Time), alpha=0.5) + 
    ggtitle("Procruste rotation comparing NMDS from index to taxa; correlation = 0.782, p=0.001") + scale_colour_manual(values = c("#006837","#fc8d59","#253494","#bd0026","#9e9ac8","#4697AB")) + theme_bw()+ theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank()) # scale_colour_manual() 